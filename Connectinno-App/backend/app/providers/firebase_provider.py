...
class FirebaseNotesRepository(NotesRepository):
    def list_notes(self, uid: str) -> List[Dict[str, Any]]:
        docs = _notes_col(uid).order_by("created_at", direction=firestore.Query.DESCENDING).stream()
        out = []
        for d in docs:
            data = d.to_dict()
            out.append({
    ...
    def create_note(self, uid: str, title: str, content: str) -> Dict[str, Any]:
        ...
        doc = {"title": title, "content": content, "created_at": now, "updated_at": now}
        ref.set(doc)
